  ...
  return (
    <li>
      <Link
        ...
        className="flex gap-4 rounded-2xl bg-surface p-4 shadow-sm transition-shadow hover:shadow-md"
      >
        {/* 이미지 썸네일 */}
        {record.images[0] ? (
          ...
          {record.tags.filter((t) => t !== "미분류").length > 0 && (
            <div className="mt-1 flex flex-wrap gap-1">
              {sortByPresetOrder([...new Set(record.tags.filter((t) => t !== "미분류"))]).map((t) => (
                <span
                  key={t}
                  className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${TAG_COLORS[t] ?? "bg-surface-muted text-text-secondary"}`}
                >
                  {t}
                </span>
              ))}
