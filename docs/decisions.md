...
## 13. GitHub Actions CI 도입

이 프로젝트는 혼자 개발하고 브랜치 없이 바로 `main`에 커밋·푸시하는 방식이라, CI가 "머지 전에 막는" 전통적인 역할을 하진 못한다. 그래도 두 가지 이유로 붙였다: (1) 로컬에서 검증할 때는 `.env.local`이 이미 채워진 상태라 놓칠 수 있는 "환경 변수 없는 깨끗한 환경에서 실패하는" 유형의 버그를 잡을 수 있고(실제로 `extractSkillFromCaption` 테스트를 준비하며 이런 버그를 이미 한 번 발견했다), (2) 커밋마다 lint·타입체크·테스트가 자동으로 통과하고 있다는 이력 자체가 남는다.
